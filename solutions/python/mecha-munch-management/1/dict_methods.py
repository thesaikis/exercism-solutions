...
def send_to_store(cart, isle_mapping):
    ...
    return {k: [cart[k], *isle_mapping[k]] for k in sorted(cart.keys(), reverse=True)}
...
def update_store_inventory(fulfillment_cart, store_inventory):
    ...
    for item in fulfillment_cart:
        store_inventory[item][0] = n if (n := store_inventory[item][0] - fulfillment_cart[item][0]) > 0 else 'Out of Stock'
